...
public class ReservationController {
    ...
    private List<Ticket> Tickets; // Store the list of Tickets
    ...
    public void initialize() {
        ...
        // Populate ComboBox with Ticket names
        ObservableList<String> TicketNames = FXCollections.observableArrayList(
                Tickets.stream()
                        .map(Ticket::getName)  // Extract the name property of each Ticket
                        .toList()
        );

        // Set items in the ComboBox
        TicketComboBox.setItems(TicketNames);
    ...
    private void handleSubmitAction() {
        try {
            ...
            String TicketName = TicketComboBox.getValue();
            ...
            if (TicketName == null || TicketName.trim().isEmpty()) {
                statusLabel.setText("Error: Please select a Ticket.");
                return;
            }

            // Find the Ticket ID based on the selected name
            int TicketId = getTicketIdFromName(TicketName);
            ...
            Reservation reservation = new Reservation(0, startTime,endTime, userId,TicketId, status);
    ...
    private int getTicketIdFromName(String TicketName) {
        // Search for the selected Ticket by name in the list of Tickets
        for (Ticket Ticket : Tickets) {
            if (Ticket.getName().equals(TicketName)) {
                return Ticket.getId();
            }
        }
        throw new IllegalArgumentException("Ticket not found: " + TicketName);
    }
    ...
    public void handleBackAction(){
        ...
        try {
            ...
            StackPane contentPane = (StackPane) TicketComboBox.getScene().lookup("#contentPane");
